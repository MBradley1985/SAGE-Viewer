...
def test_loads_correct_snap(mini_hdf5_path):
    snap = load_galaxy_snapshot(mini_hdf5_path, snap_num=63, min_stellar_mass=0.0)
...
def test_mass_cut_filters(mini_hdf5_path):
    snap_all = load_galaxy_snapshot(mini_hdf5_path, snap_num=63, min_stellar_mass=0.0)
...
def test_missing_snap_returns_empty(mini_hdf5_path):
    snap = load_galaxy_snapshot(mini_hdf5_path, snap_num=0, min_stellar_mass=0.0)
    assert snap.count == 0
